...
{
    public class StructureToSqlite : IStructure
    {
        /// <summary>
        /// 特殊数据库表名左侧字符串
        /// </summary>
        ...
        public List<EntityPropertyModel> ColumnList(SZTransaction transaction, string tableName)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                model.Att.ColumnName = row["name"].ToString();
                model.Att.ColumnType = row["type"].ToString();

                if (row["pk"].ToString().ToString() == "1")
                {
                ...
                model.Att.DefaultValue = row["dflt_value"].ToString();

                if (row["notnull"].ToString() == "0")
        ...
        public void CreateTable(SZTransaction transaction, EntityModel model)
        {
            ...
            var list = model.Fields.OrderByDescending(o => o.Att.IsKey);
        ...
        {
            throw new Exception("没有时间sqlite添加主键");
        }
        /// <summary>
        /// 删除key
        /// </summary>
        ...
        {
            return Query(selectColumns, tablename, whereSql, orderby, -1, -1);
        }
        ...
        public string Query(List<string> selectColumns, string tableName, string whereSql, Dictionary<string, string> orderby, int beginnum, int endnum)
        {
            ...
            {
                tableName = GetTableChar(tableName) + " ";
            }
            string ordersql = "  ";
            if (orderby.Any())
            {
            ...
            {
                return string.Format("SELECT {0}  FROM {1}  {2} {3}  Limit {4} Offset {5} ", selectsql, tableName, wherestr, ordersql, endnum , beginnum);
            }
        }
            ...
            switch (_field.FieldType)
            {
                ...
                case "System.Decimal":
                    ...
                    else if (_field.Att.NumberSize != 0 && _field.Att.NumberPrecision == 0)
                    {
                        ...
                        result = "NUMERIC(" + _field.Att.NumberSize + ")";
                    }
                    else
                    {
                        if (_field.Att.NumberSize < 1 || _field.Att.NumberSize > 65)
                        ...
                        result = "NUMERIC(" + _field.Att.NumberSize + "," + _field.Att.NumberPrecision + ")";
                    }
                    break;
                case "System.Byte[]":
                    result = "BLOB";
