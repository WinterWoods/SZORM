    ...
    class InternalDataReader : IDataReader, IDisposable, IDataRecord
    {
        ...
        public void Close()
        {
            if (!this._reader.IsClosed)
            {
                ...
                {
                    this._reader.Close();
                    this._reader.Dispose();/* Tips：.NET Core 的 SqlServer 驱动 System.Data.SqlClient(4.1.0) 中，调用 DataReader.Dispose() 方法后才能拿到 Output 参数值。为了与 Core 版本的代码统一，也在这执行 DataReader.Dispose() 吧 */
                }
                finally
                {
